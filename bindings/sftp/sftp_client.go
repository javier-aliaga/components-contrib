...
var log = logger.NewLogger("sftp.client")
...
func withReconnection(c *Client, fn func() error) error {
	log.Debug("withReconnection")
	err := fn()
	if err == nil {
		return nil
	}
	log.Debugf("withReconnection: error: %s", err)
	...
	log.Debug("withReconnection: after reconnect")
	err = fn()
	if err != nil {
		return err
	}

	log.Debug("withReconnection: success")
	return nil
}
...
func doReconnect(c *Client) error {
	...
	err := c.ping()
	if !shouldReconnect(err) {
		log.Debug("No reconnection needed - connection is ready")
		return nil
	}
...
func shouldReconnect(err error) bool {
	...
	if errors.Is(err, sftpClient.ErrSSHFxPermissionDenied) ||
		...
		log.Debug("No reconnection needed")
		return false
	}
